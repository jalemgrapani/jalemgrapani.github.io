        ...
        if (content.isAvailable) {
            maincontainer.innerHTML += `
                <div onclick="addToReceipt(` + content.price + `, '` + content.name + `')" 
                    ...
                    <small style="color: #68200d; font-size: 18px; display: block; margin: 0;">
                        ₱` + content.price + `
                    </small>
                </div>`;
        }
...
function addToReceipt(price, name) {
    ...
    if (!receiptContainer.querySelector(".receipt-title")) {
        receiptContainer.innerHTML = `
            <h3 class="receipt-title" style="text-align: center; margin-bottom: 10px;">Receipt</h3>` + receiptContainer.innerHTML;
    }
